    ...
    return DefaultTabController(
      ...
      child: Scaffold(
        body: SingleChildScrollView(
          child: SafeArea(
            child: BlocListener<FavoriteBloc, FavoriteStateBloc>(
              listener: (context, state) {
                if (state is FavoriteAddNickAndNumberState) {
                  ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                    content: const MyText(
                      title: "Criando Imagem Favorito",
                      fontSize: 14,
                    ),
                ...
                if (state is FavoriteUploadingState) {
                  ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                    content: const MyText(
                ...
                if (state is FavoriteCreateImageSate) {
                  ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                    content: const MyText(
                      title: "Favorito Criado com Sucesso",
                      fontSize: 14,
                    ),
                    backgroundColor: Theme.of(context).colorScheme.primary,
                    behavior: SnackBarBehavior.floating,
                      ...
                      Row(
                        ...
                        children: [
                          InkWell(
                              ...
                              child:
                                  const MyTextTitle(message: " < MAGICVIEW")),
                          BlocBuilder<FavoriteBloc, FavoriteStateBloc>(
                            builder: (context, state) {
                              if (state is FavoriteCreateImageSate ||
                                  state is FavoriteImageExistState) {
